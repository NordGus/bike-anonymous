...
class Authentication::TokenGenerator::AuthenticationTokenTest < ActiveSupport::TestCase
  test "should return a valid authentication token for the given user" do
    travel_to DateTime.new(2022, 9, 2)

    token = ::Authentication::TokenGenerator.new(user).authentication_token
  ...
  test "should return a token with the expected payload" do
    ...
    assert_equal 5.minutes.from_now.to_i, decoder.payload[:exp]
